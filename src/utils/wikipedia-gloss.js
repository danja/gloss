/*
Quick & dirty nodejs script to convert Wikipedia glossary pages from HTML into Turtle RDF
...
for (let i = 0; i < dlNodes.length; i++) {
    ...
    for (let i = 0; i < dl.childNodes.length; i++) {
        ...
        if (childNode.nodeType === 1 && childNode.nodeName === 'dd') { // 1 is an element node
            ...
            */
            addTurtle(normalize(dtNode.textContent), normalize(childNode.textContent))
        }
    }
}
